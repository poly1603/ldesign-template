...
const entryPool = new objectPool.ObjectPool({
  maxSize: 50,
  factory: () => ({
    name: "",
    startTime: 0,
...
function useTemplatePerformance(templateId) {
  ...
  const fpsHistory = vue.ref(Array.from({
  ...
  return {
    // 状态
    metrics: vue.readonly(metrics),
    entries: vue.readonly(entries),
    memory: vue.readonly(memory),
    fpsHistory: vue.readonly(fpsHistory),
    // 方法
    startMeasure,
    endMeasure,
    measure,
...
//# sourceMappingURL=useTemplatePerformance.cjs.map
