...
function useTemplateSnapshot(initialData, options = {}) {
  ...
  const currentData = vue.ref(initialData);
  const snapshots = vue.ref([]);
  const currentSnapshotIndex = vue.ref(-1);
  const isApplyingSnapshot = vue.ref(false);
  const canUndo = vue.computed(() => currentSnapshotIndex.value > 0);
  const canRedo = vue.computed(() => currentSnapshotIndex.value < snapshots.value.length - 1);
  const currentSnapshot = vue.computed(() => snapshots.value[currentSnapshotIndex.value] || null);
  const snapshotCount = vue.computed(() => snapshots.value.length);
  ...
  const unwatch = vue.watch(currentData, () => {
  }, {
    deep: true
  });
  const cleanup = () => {
  ...
  return {
    // 状态
    currentData,
    snapshots: vue.computed(() => snapshots.value),
    currentSnapshot,
    currentSnapshotIndex: vue.computed(() => currentSnapshotIndex.value),
    snapshotCount,
    // 时间旅行
    canUndo,
    canRedo,
...
exports.useTemplateSnapshot = useTemplateSnapshot;
exports.useTimeTravel = useTimeTravel;
/*! End of @ldesign/template | Powered by @ldesign/builder */
//# sourceMappingURL=useTemplateSnapshot.cjs.map
