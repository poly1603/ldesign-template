...
class TemplateLoader {
  ...
  async load(category, device, name, options) {
    ...
    const template = scanner$1.getTemplate(category, device, name);
    if (!template) {
      const error = new Error(`\u6A21\u677F\u672A\u627E\u5230: ${key}`);
      options?.onError?.(error);
      throw error;
    }
    styleLoader.loadComponentStyle(category, device, name, template.componentPath);
  ...
  async preloadByFilter(filter) {
    const scanner$1 = scanner.getScanner();
    const allMetadata = scanner$1.getAllMetadata();
