...
function useTemplateLifecycle(category, device, name, lifecycle, options) {
  ...
  async function loadTemplate() {
    try {
      ...
      const loadedComponent = await manager.loadTemplate(categoryRef.value, deviceRef.value, nameRef.value, options);
  ...
  const stopWatcher = watch([categoryRef, deviceRef, nameRef], () => loadTemplate(), {
    immediate: false
  });
...
function useTemplateNavigation() {
  ...
  const current = computed(() => currentIndex.value >= 0 ? history.value[currentIndex.value] : null);
  const clearHistory = () => {
    history.value = [];
...
function useTemplatePerformance() {
  const metrics = ref(/* @__PURE__ */ new Map());
...
export { useTemplateLifecycle, useTemplateNavigation, useTemplatePerformance, useTemplatePrefetch };
/*! End of @ldesign/template | Powered by @ldesign/builder */
//# sourceMappingURL=useTemplateHooks.js.map
