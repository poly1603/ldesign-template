...
export { MemoryLeakDetector, getLeakDetector, safeRequestAnimationFrame, safeSetInterval, safeSetTimeout, useAutoCleanupListener, useMemoryLeakDetection } from './memoryLeakDetector.js';
export { ObjectPool, createArrayPool, createMapPool, createObjectPool, createSetPool, poolManager } from './objectPool.js';
...
class TemplateError extends Error {
  constructor(message, type = TemplateErrorType.UNKNOWN, options) {
    super(message);
  ...
  return defineComponent({
    ...
    setup(props, {
      ...
      const handleError = async (err) => {
        const templateError = err instanceof TemplateError ? err : new TemplateError(err.message, TemplateErrorType.UNKNOWN, {
          cause: err
        });
      ...
      const renderError = () => {
        ...
        return h("div", {
          ...
          class: "error-icon"
        }, "\u26A0\uFE0F"), h("h3", error.value?.getUserMessage()), h("p", {
          class: "error-details"
        }, error.value?.message), error.value?.retryable && h("button", {
          class: "error-retry-btn",
          onClick: retry
        }, recovering.value ? "\u6062\u590D\u4E2D..." : "\u91CD\u8BD5"), h("details", {
          class: "error-suggestions"
        }, [h("summary", "\u89E3\u51B3\u5EFA\u8BAE"), h("ul", error.value?.getRecoverySuggestions().map((suggestion) => h("li", suggestion)))])]);
      };
...
export { ErrorRecoveryManager, GlobalTemplateErrorHandler, TemplateError, TemplateErrorType, createErrorBoundary, defaultRecoveryStrategies, globalErrorHandler };
